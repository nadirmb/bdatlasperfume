...
//botoness para restar y eliminar i no quiero
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_perfume'], $_POST['accion'])) {
$id = $_POST['id_perfume'];

if ($_POST['accion'] === 'sumar') {
$_SESSION['cesta'][$id] = ($_SESSION['cesta'][$id] ?? 0) + 1;
}
...
if (!isset($_SESSION['cesta']) || empty($_SESSION['cesta'])) {
    ...
    foreach ($_SESSION['cesta'] as $id_perfume => $cantidad) {
        ...
        $productosHTML .= "
            <form method='POST' class='list-group-item d-flex justify-content-between align-items-center border-bottom py-3'>
             <input type='hidden' name='id_perfume' value='{$id_perfume}'>
             <div>
                <strong>{$nombre}</strong> x{$cantidad}
             </div>
             <div>
                <span style='margin-right: 10px;'>{$subtotal} euros</span>
                <button name='accion' value='sumar' style='border: 1px solid #ccc; background: none; padding: 4px 8px;'>+</button>
                <button name='accion' value='restar' style='border: 1px solid #ccc; background: none; padding: 4px 8px;'>-</button>
                <button name='accion' value='eliminar' style='border: 1px solid #ccc; background: none; padding: 4px 8px;'>Eliminar</button>
             </div>
            </form>";
    }
}
